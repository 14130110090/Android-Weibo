...
import com.weibo.listener.ListenerManager.onDiaryClickCollectionListener;
...
public class CollectionActivity extends Activity implements LoadHomeListener,
		onDiaryClickCollectionListener {
	...
	private void initView() {
		...
		adapter = new HomeFragmentAdapter(CollectionActivity.this, arrayList,
				mLruCache, fileCache);
		// 必须设置，否则就默认为0,也就是默认为关注页面创建的adapter
		adapter.setFlag(2);
		Button collection_back = (Button) findViewById(R.id.collection_back);
		pullLayout = (PullToRefreshLayout) findViewById(R.id.collection_pulltorefresh);
		listView = (PullableListView) findViewById(R.id.collection_list);
		collection_back.setOnClickListener(new OnClickListener() {
	...
	public void getData(int start, int end) {
		LoadHomeTask home = new LoadHomeTask(CollectionActivity.this);
	...
	public void LoadJson(JSONObject json, int start_index) {
		try {
			if (json != null) {
				...
				if (json.getString("action").equals(GETCOLLECTION_SUCCESS)) {
					...
					new Thread() {
						...
						public void run() {
							for (int i = 0; i < arrayList.size(); i++) {
								...
								// 先假定最多保存25条数据
								if (LocalAccessUtil
										.getDiaryCount(CollectionActivity.this) < 25) {
									...
									int diary_id = LocalAccessUtil
											.getLeastUsedDiary(CollectionActivity.this);
									LocalAccessUtil.deleteDiary(
											CollectionActivity.this, diary_id);
									LocalAccessUtil.saveDiary(
											CollectionActivity.this,
											arrayList.get(i));
								}
